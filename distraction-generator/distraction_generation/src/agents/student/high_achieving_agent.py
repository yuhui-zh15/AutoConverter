...
class HighAchievingAgent(Agent):
    def __init__(self, model: str, api_key: str):
        super().__init__(model, api_key)

    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        test_prompt = get_prompt("high_achieving_system_prompt")
        ...
        high_achieving_responses = json.dumps(
            [self._test_to_dict(option) for option in reply.parsed.options], indent=2
        )

        return {"high_achieving_responses": high_achieving_responses}
