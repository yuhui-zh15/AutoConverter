...
class KnowledgeDeficientAgent(Agent):
    def __init__(self, model: str, api_key: str):
        super().__init__(model, api_key)

    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        test_prompt = get_prompt("knowledge_deficient_system_prompt")
        ...
        knowledge_deficient_responses = json.dumps(
            [self._test_to_dict(option) for option in reply.parsed.options], indent=2
        )

        return {"knowledge_deficient_responses": knowledge_deficient_responses}
